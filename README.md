...
모바일 브라우저에서는 화면 아무 곳이나 탭하면 하드 드롭, 좌우로 드래그하면 좌우 이동, 위쪽 대각선 드래그로 회전합니다. 위로 거의 직선 드래그하면 기본적으로 시계 방향 회전이며, 오른쪽 위는 시계 방향, 왼쪽 위는 반시계 방향입니다. 우상단 **MENU** 버튼은 일시정지와 메인 메뉴 이동에 사용합니다.
